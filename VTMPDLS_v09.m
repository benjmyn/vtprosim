...
disp("Walking points...")
[MVWP,GX,GY] = walk(Car(1),SWP,VWP,RWP);
    ...
    for n = 1:length(SWP)
        ...
        if VWP(n) < MVWP(n)
            ...
            for i = (n+1):length(SWP)
                ...
                a(i) = real(sqrt(Car.axT_fcn(UWP(i-1)).^2 - (Car.ax_scale.*UWP(i-1).^2./RWP(i-1)).^2));
            ...
            for i = 1:(n-1)
                ...
                a(j) = -real(sqrt(Car.axB_fcn(UWP(j+1)).^2 - (Car.ax_scale.*UWP(j+1).^2./RWP(j+1)).^2));
